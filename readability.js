 ...
 *
 * Code modifed for use with the Hacked News browser extension by Bala Clark, 2013
 *
 * Copyright (c) 2009 Arc90 Inc
 * Readability is licensed under the Apache License, Version 2.0.
 *
 * TODO: replace relative image srcs (init with base url)
**/
var readability = {
	version:     '0.5.1',
		...
		**/
		if(readability.getInnerText(articleContent, false) == "")
		{
			if(!preserveUnlikelyCandidates) {
				this.document.body.innerHTML = readability.bodyCache;
				return readability.init(document, true);
			}
			else
			{
	...
	getArticleTitle: function () {
		var articleTitle = this.document.createElement("H1");
	...
	prepDocument: function () {
		...
		this.document.body.innerHTML = this.document.body.innerHTML.replace(readability.regexps.replaceBrsRe, '</p><p>').replace(readability.regexps.replaceFontsRe, '<$1span>')
	},
